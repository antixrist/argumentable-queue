...
var memoinesic = function(func, ttl, hasher) {
  var memoize;
  ...
  memoize = function() {
    ...
    now = Date.now();
    if (!_.has(cache, key) || now > cache[key].expires) {
      cache[key] = {
...
Queue.prototype.add = function Queue$add () {
  ...
  this._updatePrioritiesByNewTask(this.tasks[key]);
...
Queue.prototype._setupTask = function Queue$_setupTask (task, options) {
  ...
  if (task) {
    ...
    this.tasks[task.key].priority = parseInt(this.tasks[task.key].priority, 10);
    this.tasks[task.key].priority = this.tasks[task.key].priority || this.options.defaultPriority;

    this._updatePrioritiesByNewTask(this.tasks[task.key], previousPriority);
  }
 ...
 * @private
 */
Queue.prototype._updatePrioritiesByNewTask = function Queue$_updatePrioritiesByNewTask (task, previousPriority) {
...
Queue.prototype._getTimeForNextTask = function Queue$_getTimeForNextTask () {
  var now = Date.now();
  var ttl = 20;
  this.cachedTimeForNextTask = this.cachedTimeForNextTask || now;

  if ((now - this.cachedTimeForNextTask) >= ttl) {
 ...
 * @returns {boolean}
 */
Queue.prototype.waitForNextTaskAllowed = function Queue$waitForNextTaskAllowed () {
  nextTick(function () {
