...
Queue.prototype._next = function Queue$_next () {//console.log.apply(console, ['= _next'].concat(arguments));
  ...
  this.taskInOrder = this.taskInOrder || this._findNextTask();
  var allowed = this._nextTaskAllowed();
  console.log('next allowed', allowed);
  ...
  if (allowed) {
    if (this.taskInOrder) {
      ...
      let taskKey = this.taskInOrder.key;
      this.taskInOrder = null;
  ...
  //} else if (!this.taskInOrder) {
  //  setTimeout(() => { this._next(); }, this._getTimeForNextTask() + 10);
  }
