...
methods.calcTimeForNextTask = function Queue$calcTimeForNextTask () {
  ...
  var byThrottle = (throttle && this.lastTaskTimes.start) ? throttle - (now - this.lastTaskTimes.start) : 0;
  //var byDebounce = (debounce && this.lastTaskTimes.end) ? debounce - (now - this.lastTaskTimes.end) : 0;

  var byDebounce = 0;
  if (this.lastTaskTimes.end) {
    byDebounce = debounce - (now - this.lastTaskTimes.end);
  } else {
  ...
  //  //var timeForEqual = self.lastTaskTimes.end || self.lastTaskTimes.start || 0;
  //  //return debounce + (now - timeForEqual);
  //
  //  if ((now - self.lastTaskTimes.end) < debounce) {
  //    return debounce + (now - self.lastTaskTimes.end);
  //  }
  //
  //  return 0;
  //})();
  ...
  if (byDebounce > 0 && this.inProgressCount <= methods.getConcurrency.call(this)) {
    ...
    //byDebounce = (debounce && this.lastTaskTimes.start) ? debounce - (now - this.lastTaskTimes.start) : 0;
  }
...
Object.defineProperty(Queue.prototype, 'stats', {
  get: function () {
    return {
      all: this.size,
      new: _.filter(this.tasks, (task) => methods.getStatus.call(this, task) == Queue.STATUS_NEW).length,
      pending: _.filter(this.tasks, (task) => methods.getStatus.call(this, task) == Queue.STATUS_PENDING).length,
      finished: _.filter(this.tasks, (task) => methods.getStatus.call(this, task) == Queue.STATUS_DONE).length
    };
  }
});
