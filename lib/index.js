...
Queue.prototype._next = function Queue$_next () {
  console.log('===');
  console.log('= _next()');
  console.log('  this.waitingForNextTaskTimeout', !!this.waitingForNextTaskTimeout);
  ...
  if (this.nextTask) {
    if (allowed) {
      ...
      // todo: продумать кеш для _getTimeForNextTask! ... или всё уже продумано?
  ...
  //    this.emit && this.emit('empty');
  //  }
  //} else if (!this.waitingForNextTaskTimeout && this.nextTask) {
  //  this.waitingForNextTaskTimeout = true;
...
Queue.prototype._runTask = function Queue$_runTask (task) {
  ...
  console.log('= _runTask()', this.runnedCount, this.finishedCount, this.size);
  ...
  functionDone(function (done) {
    ...
    return function (err, result) {
      console.log('= functionDone done()', self.runnedCount, self.finishedCount, self.size);
