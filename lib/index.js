 ...
 * @returns {Function}
 */
var taskRunner = function taskRunner (taskKey) {
  /**
   * @this {Queue}
   */
  return function (done) {
    var task = this.tasks[taskKey];
    this.emit && this.emit.call(this, 'task:start', this.getByKey(task.key));

    var retVal = this.fn.apply(this.options.ctx || null, task.args.concat(done));
 ...
 * @returns {Function}
 */
var taskRunnerCallback = function taskRunnerCallback (taskKey) {
  /**
   * @this {Queue}
   */
  return function (err, result) {
    var now                = Date.now();
    var task               = this.getByKey(taskKey);
    task.end               = now;
    task.time              = task.end - task.start;
    task.status            = Queue.STATUS_DONE;
    this.lastTaskTimes.end = now;

    ++this.finishedCount;
    --this.pendingCount;

    if (!this.options.history) {
      methods.delete.call(this, task);
    } else {
      methods.deleteTaskFromPriorities.call(this, task);
    }

    this.emit && this.emit.call(this, 'task:done', err,
