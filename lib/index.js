...
methods.calcTimeForNextTask = function Queue$calcTimeForNextTask () {
  ...
  if (debounce) {
    byDebounce = debounce - (now - (this.lastTimeFinished ? this.lastTimeFinished : this.lastTimeStart));
    ...
    let newCount = this.stats[Queue.STATUS_NEW].length;
...
Object.defineProperty(Queue.prototype, 'stats', {
  ...
  get: function () {
    ...
    Object.defineProperties(stat, {
      time: {
        get: function () {
          return (!self.options.history) ? 0 : _.reduce(self.tasks, (time, task) => {
