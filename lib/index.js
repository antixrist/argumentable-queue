...
Queue.prototype.has = function Queue$has () {
  var args = slice(arguments),
      hash = getHash(args);
  log('has hash', hash, args);

  return this.tasks[hash] || false;
};
...
Queue.prototype.get = function Queue$get () {
  ...
  log('get hash', getHash(args), args);
