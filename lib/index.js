...
methods.nextTaskAllowed = function Queue$nextTaskAllowed () {
  var concurrency     = methods.getConcurrency.call(this, true);
  var timeForNextTask = methods.getTimeForNextTask.call(this, true);

  return (
    this.pendingCount < concurrency &&
    !timeForNextTask
  );
};
...
methods.getConcurrency = function Queue$getConcurrency (forceCache) {
  ...
  this.concurrencyCache = (typeof this.concurrencyCache != 'undefined')
...
methods.calcTimeForNextTask = function Queue$calcTimeForNextTask () {
  ...
  var byThrottle = this.lastTaskTimes.start ? throttle - (now - this.lastTaskTimes.start) : 0;
  var byDebounce = this.lastTaskTimes.end   ? debounce - (now - this.lastTaskTimes.end)   : 0;

  // маленький хак, чтобы набить очередь.
  // иначе, по факту, в один момент времени будет выполняться только одна задача
  if (byDebounce && this.pendingCount <= methods.getConcurrency.call(this)) {
    byDebounce = 0;
  }
...
methods.findNextTask = function Queue$findNextTask () {
  ...
  if (!!this.priorities[this.leadPriority]) {
    ...
    taskKey = _.find(priorityQueue, (_taskKey) => {
      var _task = methods.getByKey.call(this, _taskKey);
      return !!(this.isTask(_task) && _task.status == Queue.STATUS_NEW);
    });
...
var tmp1 = function (ctx, taskKey) {
  return function (done) {
    var task = ctx.tasks[taskKey];
    ctx.emit && ctx.emit.call(ctx, 'task:start', ctx.getByKey(task.key));

    var retVal = ctx.fn.apply(ctx.options.ctx || null, task.args.concat(done));
...
var tmp2 = function (ctx, taskKey) {
  return function (err, result) {
    var now                = Date.now();
    var task               = ctx.getByKey(taskKey);
    task.end               = now;
    task.time              = task.end - task.start;
    task.status            = Queue.STATUS_DONE;
    ctx.lastTaskTimes.end = now;
    ...
    if (!ctx.options.history) {
      deleted = true;
      methods.delete.call(ctx, task);
    } else {
      methods.deleteTaskFromPriorities.call(ctx, task);
    }

    ctx.emit && ctx.emit.call(ctx,
      'task:done'
      ,err,
      (typeof result != 'undefined') ? result : void 0,
      (deleted) ? task : ctx.getByKey(task.key)
    );

    task = null;
    methods.next.call(ctx);
  };
};
...
methods.runTask = function Queue$runTask (task) {
  ...
  functionDone(tmp1(this, task.key), tmp2(this, task.key));
};
