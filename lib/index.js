...
methods.calcTimeForNextTask = function Queue$calcTimeForNextTask () {
  ...
  var byDebounce = debounce && (function () {
    var timeForEqual = (self.lastTaskTimes.end) ? self.lastTaskTimes.end : self.lastTaskTimes.start;
  ...
  var retVal =  _.max(_.map(times, function (time) {
    // отрицательные значения не возвращаем
    return (time < 0) ? 0 : time;
  }));

  console.log('delay', retVal);
