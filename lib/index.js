...
Queue.prototype.byKeyDelete = function Queue$byKeyDelete (key) {
  ...
  if (this._delete(task)) {
    this.emit && this.emit.call(this, 'task:delete', this.getByKey(task));
  }
...
Queue.prototype._next = function Queue$_next () {//console.log.apply(console, ['= _next'].concat(arguments));
  ...
  var nextTaskAllowed = this._nextTaskAllowed();

  console.log('   _next nextTaskAllowed', nextTaskAllowed);
  ...
  if (nextTaskAllowed) {
    ...
    if (!task) {
      ...
      nextTick((taskKey) => {
        this.taskInOrder = null;
        this._runTask(this.tasks[taskKey]);
      }, task.key);
    }
  }
...
Queue.prototype._nextTaskAllowed = function Queue$_nextTaskAllowed () {//console.log.apply(console, ['= _nextTaskAllowed'].concat(arguments));
  ...
  console.log('  this.options.concurrency', this._getConcurrency());
...
Queue.prototype._calcTimeForNextTask = function Queue$_calcTimeForNextTask () {//console.log.apply(console, ['= _calcTimeForNextTask'].concat(arguments));
  ...
  var times = [];
  times.push(this.lastTaskTimes.start ? now - this.lastTaskTimes.start - throttle : 0);
  times.push(this.lastTaskTimes.stop  ? now - this.lastTaskTimes.stop  - debounce : 0);
...
Queue.prototype._runTask = function Queue$_runTask (task) {
  //console.log.apply(console, ['= _runTask'].concat(arguments));
  var self = this;
