...
Queue.prototype._refreshLeadPriority = function Queue$_refreshLeadPriority () {console.log.apply(console, ['= _refreshLeadPriority'].concat(arguments));
  this.leadPriority = _.min(_.keys(this.priorities)) || this.options.defaultPriority;
 ...
 * @returns {Queue}
 */
Queue.prototype._next = function Queue$_next () {console.log.apply(console, ['= _next'].concat(arguments));
  if (this.isPaused()) { return this; }

  console.log('  _next this._nextTaskAllowed()', this._nextTaskAllowed());

  if (this._nextTaskAllowed()) {
    /** @type {Queue~task} */
    var task = this._extractNextTask();
    if (!task) {
      // т.к. _next() вызывается "хаотично" из разных мест по разным причинам,
      ...
      // если запускается новый таск, и увеличиваться перед emit'ом события `empty`.
 ...
 * @returns {boolean}
 */
Queue.prototype._nextTaskAllowed = function Queue$_nextTaskAllowed () {console.log.apply(console, ['= _nextTaskAllowed'].concat(arguments));
  console.log('  this.taskInOrder', this.taskInOrder);
  console.log('  this.isPaused()', this.isPaused());
  console.log('  this.runnedCount', this.runnedCount);
  console.log('  this.options.concurrency', _result(this.options, 'concurrency', this));
  console.log('  this._getTimeForNextTask()', this._getTimeForNextTask());

  return (
    //this.tasks.length &&
    !this.taskInOrder &&
    !this.isPaused() &&
    this.runnedCount < this._getConcurrency() &&
...
Queue.prototype._findNextTask = function Queue$_findNextTask () {console.log.apply(console, ['= _findNextTask'].concat(arguments));
  ...
  if (!!this.priorities[this.leadPriority]) {
    ...
    if (!task) {
      ...
      // обновляем очереди по приоритету (там же обновится и `this.leadPriority`, и удалятся пустые приоритты (проверки на _isNew там нет!))
      this._tidyPriorities();

      // и рекурсивно ищем таск по новой
      if (this.priorities[this.leadPriority]) {
        task = this._findNextTask();
      }
    }
  } else {
    this._tidyPriorities();
  ...
  if (!task) {
    // то удаляем очередь с этим приоритетом (ибо в ней всё-равно для постановки в очередь ничего нету)
    delete this.priorities[this.leadPriority];
    // обновляем очереди по приоритету (там же обновится и `this.leadPriority`)
    this._tidyPriorities();
...
Queue.prototype._deleteTaskFromPriorities = function Queue$_deleteTaskFromPriorities (task) {
  console.log.apply(console, ['= _deleteTaskFromPriorities'].concat(arguments));
...
Queue.prototype._runTask = function Queue$_runTask (task) {
  ...
  functionDone(function (done) {
    console.log('functionDone task:', task);
    var now = Date.now();
    ...
    return function (err, result) {
      console.log('functionDone task done:', task);
      task.end               = now;
...
Queue.prototype._getTimeForNextTask = function Queue$_getTimeForNextTask () {console.log.apply(console, ['= _getTimeForNextTask'].concat(arguments));
  ...
  //console.log('_getTimeForNextTask time', time);
...
Queue.prototype._calcTimeForNextTask = function Queue$_calcTimeForNextTask () {console.log.apply(console, ['= _calcTimeForNextTask'].concat(arguments));
  ...
  //console.log('_calcTimeForNextTask times', times);
