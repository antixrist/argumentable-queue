...
methods.nextTaskAllowed = function Queue$nextTaskAllowed () {
  ...
  return (
    this.inProgressCount < concurrency &&
    !timeForNextTask
  );
};
...
methods.calcTimeForNextTask = function Queue$calcTimeForNextTask () {
  ...
  if (byDebounce && this.inProgressCount <= methods.getConcurrency.call(this) - 1) {
    byDebounce = 0;
  }
  ...
  return _.max(times) || 0; // отрицательные значения не возвращаем
};
