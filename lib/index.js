 ...
 * @returns {Number}
 */
Queue.prototype._refreshLeadPriority = function Queue$_refreshLeadPriority () {
...
Queue.prototype._nextTaskAllowed = function Queue$_nextTaskAllowed () {
  var concurrency     = this._getConcurrency(true);
  var timeForNextTask = this._getTimeForNextTask(true);
 ...
 * @returns {number}
 */
Queue.prototype._getConcurrency = function Queue$_getConcurrency (forceCache) {
  if (!!forceCache) {
    this.concurrencyCache = void 0;
  }
 ...
 * @returns {number}
 */
Queue.prototype._getTimeForNextTask = function Queue$_getTimeForNextTask (forceCache) {
  if (!!forceCache) {
    this.timeForNextTaskCache = void 0;
  }
...
Queue.prototype._calcTimeForNextTask = function Queue$_calcTimeForNextTask () {
  ...
  //if (byDebounce && this.runnedCount < this._getConcurrency()) {
  //  byDebounce = 0;
  //}

  //console.log('byDebounce', byDebounce);
  console.log('this._getConcurrency()', this._getConcurrency());
 ...
 * @returns {?Queue~task}
 */
Queue.prototype._findNextTask = function Queue$_findNextTask () {
  var priorityQueue;
